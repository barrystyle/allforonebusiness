...
PIVXGUI::PIVXGUI(const NetworkStyle* networkStyle, QWidget* parent) :
    ...
    if (windowTitle.isEmpty()) {
        windowTitle = tr("AllForOneBusiness Core") + " - ";
...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("AllForOneBusiness Core client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("AllForOneBusiness Core"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("AllForOneBusiness Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
